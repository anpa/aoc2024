defmodule Day06 do
  ...
  defp visited?(visited, {x, y}, dir) do
  ...
  defp stuck_in_loop?(map, obstacle_pos, pos, dir, visited) do
    ...
    dir_n =
      if map[yn][xn] == "#" || obstacle_pos == {yn, xn} do
        next_dir(dir)
      else
        dir
      end
  ...
  def part_2(file_path) do
    ...
    |> Enum.filter(fn obstacle_pos ->
      stuck_in_loop?(map, obstacle_pos, init_pos, dir, visited)
    end)
    |> length()
  end
end
...
IO.puts("Part 2 - input: #{ret_val} (in #{time_in_microseconds / 1_000_000} seconds)")
